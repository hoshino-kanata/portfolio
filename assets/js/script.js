...
/* =============================
   Liquid Glass Navigation Animation
============================= */
document.addEventListener("DOMContentLoaded", () => {
  // SVGフィルターが存在するページのみ実行
  if (!document.getElementById('fe-image-nav')) return;
  ...
  const navConfig = {
    ...
    radius: 40,
    border: 0.1,
    lightness: 65, // ガラスの明るさ
    alpha: 0.8,
    blur: 15,
    scale: -40     // ベースの歪み強度
  };

  const buildNavDisplacementImage = () => {
    const border = Math.min(navConfig.width, navConfig.height) * (navConfig.border * 0.5);
    // 赤と青のグラデーションでX軸/Y軸の歪みマップを作る
    const svgString = `
      <svg viewBox="0 0 ${navConfig.width} ${navConfig.height}" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <linearGradient id="nav-red" x1="100%" y1="0%" x2="0%" y2="0%">
            <stop offset="0%" stop-color="#000"/>
            <stop offset="100%" stop-color="red"/>
          </linearGradient>
          <linearGradient id="nav-blue" x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stop-color="#000"/>
            <stop offset="100%" stop-color="blue"/>
          </linearGradient>
        </defs>
        <rect x="0" y="0" width="${navConfig.width}" height="${navConfig.height}" fill="black"></rect>
        <rect x="0" y="0" width="${navConfig.width}" height="${navConfig.height}" rx="${navConfig.radius}" fill="url(#nav-red)" />
        <rect x="0" y="0" width="${navConfig.width}" height="${navConfig.height}" rx="${navConfig.radius}" fill="url(#nav-blue)" style="mix-blend-mode: difference" />
        <rect x="${border}" y="${border}" width="${navConfig.width - border * 2}" height="${navConfig.height - border * 2}" rx="${navConfig.radius}" fill="hsl(0 0% ${navConfig.lightness}% / ${navConfig.alpha})" style="filter:blur(${navConfig.blur}px)" />
      </svg>
    `;
    ...
    gsap.set('#fe-image-nav', { attr: { href: dataUri } });
    gsap.set('#fe-displace-nav', { attr: { scale: navConfig.scale } });
  };

  buildNavDisplacementImage();

  // ゆっくりと波打つ（アンニュイな）アニメーション
  gsap.to(navConfig, {
    scale: -60, // ここまでの間で屈折率が変化する
    duration: 4,
    ease: "sine.inOut",
    yoyo: true,
    repeat: -1,
